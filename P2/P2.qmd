...
# New Predictors section

### New Predictor: Home Ownership

Homeowners and renters may face different transportation needs and constraints. Homeownership is often associated with longer residence and investment in auto ownership, whereas renters (especially in urban areas) may opt to rely on alternative transportation.

```{r}
hh_data <- hh_data %>%
  mutate(homeowner = case_when(
    HOMEOWN == "01" ~ "Own",
    HOMEOWN %in% c("02", "97") ~ "Rent",
    TRUE ~ NA_character_   # For nonresponses (-7, -8) or other unexpected codes
  )) %>%
  mutate(homeowner = factor(homeowner, levels = c("Own", "Rent")))
```

### New Predictor: Urban/Rural Classification (URBAN or URBRUR)

Even though we already have a density variable, a direct indicator of whether the household is in an urban or rural area may capture additional differences (for example, rural households are often more vehicle dependent than urban ones).

```{r}
hh_data <- hh_data %>%
  mutate(urban_rural = case_when(
    URBAN %in% c("01", "02", "03") ~ "Urban",
    URBAN == "04" ~ "Rural",
    TRUE ~ NA_character_
  )) %>%
...
### New Predictor: **Educational Attainment (EDUC):**

Education level can serve as a proxy for socioeconomic status and lifestyle. Households with higher education levels might have different travel preferences (for example, a greater willingness to use alternative modes or oppoosite) compared to those with lower education levels.

```{r}
household_educ <- person_data %>%
  # Keep only valid education responses.
  filter(EDUC %in% c("01", "02", "03", "04", "05")) %>%
  group_by(HOUSEID) %>%
  summarise(
    educ_level = factor(
      max(as.numeric(EDUC), na.rm = TRUE),
      levels = 1:5,
    ...
    .groups = "drop"  # This ungroup()s the data automatically.
  )

# Join the recoded education variable
hh_data <- hh_data %>%
  left_join(household_educ, by = "HOUSEID")
```

## Preparing data

### Boston Region's vehicle availability data

```{r}
hh_data_original <- hh_data |>
  select(HOUSEID,
         veh_avail,
         WRKCOUNT,
         n_child,
...
### My Data with Additional Predictors
...
#Original Data
hh_data_train_ids_original <- sample(hh_data_original$HOUSEID, 
                        size = ceiling(nrow(hh_data)/2))

hh_data_train_original <- hh_data_original |>
  filter(HOUSEID %in% hh_data_train_ids_original)

hh_data_test_original <- hh_data_original |>
  filter(HOUSEID %!in% hh_data_train_ids_original)
...
```{r}
veh_dfidx_train_original <- fn_make_dfidx(hh_data_train_original,
                                "HOUSEID",
                                "veh_avail")

veh_dfidx_test_original <- fn_make_dfidx(hh_data_test_original,
                                "HOUSEID",
                                "veh_avail")
...
```{r}
model_veh_original <- mlogit(choice ~ 0 | 
                      WRKCOUNT +
                      n_child +
                      n_seniors +
                      n_extra_drivers +
...
predicts_test_original <- predict(model_veh_original, veh_dfidx_test_original) |>
  as.data.frame() |>
...
```{r}
predicts_test_original <- predicts_test_original |>
...
```{r}
confusionMatrix(data = predicts_test_original$most_likely,
                reference = predicts_test_original$veh_avail)
```
